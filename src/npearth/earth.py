...
class EARTH:
    def __init__(
        ...
        """
        Multivariate Adaptive Regression Splines (MARS) regressor [Friedman, J. (1991)].
        ...
        prune_model : bool, default=True
            If True, run the backward‐pass pruning procedure after the forward step. Prunes using the CGV score in [Friedman, J. (1991)]
    ...
    def score(
        ...
        v = (((y - y.mean()) ** 2) * sample_weight).sum()
        if v > 0:
            return 1 - u / v
        else:
            return np.nan
